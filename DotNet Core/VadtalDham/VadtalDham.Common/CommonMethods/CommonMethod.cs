...
using System.Web;
using VadtalDham.Model.Error;

namespace VadtalDham.Common.CommonMethods
{
    public class CommonMethod
    {
        ...
        public static string GetKeyValues(HttpContext context)
        {
            ...
            {
                List<ReqResKeyValue> lstReqResponse = new List<ReqResKeyValue>();

                var data = context.Request.Form.ToDictionary(x => x.Key, x => x.Value.ToString()).ToList();

                for (int i = 0; i < data.Count - 1; i++)
                {
                    ReqResKeyValue obj = new ReqResKeyValue();
